...
func (p *ImageProcessor) processCDN(ctx context.Context, src io.Reader, pageID uint, ownerID uint, imageKind uploadentity.ImageKind) (param.ProcessImageResult, error) {
	const op = richerror.Op("imageprocessor.processCDN")
	...
	result, err := p.storage.Save(ctx, storage.SaveRequest{
		File:     &buf,
		Filename: fmt.Sprintf("conversions/%d-cdn.jpg", pageID),
		OwnerID:  ownerID,
		Kind:     imageKind,
		MimeType: "image/jpeg",
		Size:     int64(buf.Len()),
	...
	if err != nil {
		return param.ProcessImageResult{}, richerror.New(op).
			WithErr(err).
			WithMessage("failed to save cdn variant").
			WithKind(richerror.KindUnexpected)
	}

	return param.ProcessImageResult{
		Kind:       entity.VariantKindCDN,
		ImageURL:   result.URL,
